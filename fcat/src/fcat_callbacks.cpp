...
void Fcat::AsyncSdoReadCmdCb(const std::shared_ptr<fcat_msgs::msg::AsyncSdoReadCmd> msg) {
  fastcat::DeviceCmd cmd{};  // value-init: zero all fields (some cmd fields, e.g. prof_pos end_velocity, are not set below)
  cmd.name = msg->name;
...
void Fcat::CallActuatorProfPos(const fcat_msgs::msg::ActuatorProfPosCmd& msg) {
  ...
  // fcat's ROS ActuatorProfPosCmd has no end_velocity field; fastcat validates
  // it against the speed limit, so it must be a sane value (0 = stop at goal),
  // not the uninitialized garbage it would otherwise hold.
...
void Fcat::ActuatorProfPosSrvCb(
  ...
  // fcat's ProfPos service has no end_velocity field; fastcat validates it
  // against the speed limit, so it must be a sane value (0 = stop at goal).
  cmd.actuator_prof_pos_cmd.end_velocity = 0.0;
  response->success = ActuatorExistsOnBus(cmd.name, response->message);
  if (response->success) {
    QueueCommand(cmd);
